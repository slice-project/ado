...
public class Predator1Impl extends AbstractAgent {
	
	public Predator1Impl() {

	}

	@SuppressWarnings("unchecked")
	public INDArray getObservations() throws ObservationNotFound {
		...
		Optional<? extends Tuple> agent_loc = tuples.get("predator1-loc");
		if ( !agent_loc.isPresent() ) throw new ObservationNotFound("obs[key=predator1-loc] is not found");
		
		obs[0][2] = ((Pair<Float, Float>) agent_loc.get()).getValue0();
		obs[0][3] = ((Pair<Float, Float>) agent_loc.get()).getValue1();
				
		Optional<? extends Tuple> agent1_loc = tuples.get("predator2-loc");
		if ( !agent_loc.isPresent() )  throw new ObservationNotFound("obs[key=predator2-loc] is not found");
		...
		if ( !agent2_loc.isPresent() )  throw new ObservationNotFound("obs[key=predator3-loc] is not found");
		...
		if ( !agent3_loc.isPresent() )  throw new ObservationNotFound("obs[key=prey-loc] is not found");
		...
		if ( !agent3_vel.isPresent() )  throw new ObservationNotFound("obs[key=prey-velocity] is not found");
